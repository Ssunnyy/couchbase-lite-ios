...
- (nullable CBLArray*) arrayForKey: (NSString*)key {
    return $castIf(CBLArray, [self fleeceValueToObjectForKey: key]);
}
...
- (nullable CBLDictionary*) dictionaryForKey: (NSString*)key {
    return $castIf(CBLDictionary, [self fleeceValueToObjectForKey: key]);
}
...
- (CBLFragment*) objectForKeyedSubscript: (NSString*)key {
    return [[CBLFragment alloc] initWithValue: [self fleeceValueToObjectForKey: key]];
}
...
- (id) cbl_toCBLObject {
    return [[CBLMutableDictionary alloc] initWithFleeceData: self.data];
}
